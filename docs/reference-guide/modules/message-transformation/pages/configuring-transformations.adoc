...
When _several_ registered transformations could match the _same_ stored event, the chain resolves it like Java overload resolution: the *most specific* match wins.
An exact identity match (a concrete `from`) always wins over a xref:defining-transformations.adoc#_matching_with_a_predicate[predicate `from`], independent of registration order.
