...
[WARNING]
.Upcaster support is being introduced incrementally
====
The full upcaster API described below is not yet available in Axon Framework 5.2.0. It is being reintroduced (see https://github.com/AxonFramework/AxonFramework/issues/3597[issue #3597]) with updated APIs aligned with the new conversion architecture.

Rely on xref:#event_versioning_at_handling_time[payload conversion at handling time] for most versioning scenarios.
ifdef::advanced-framework[]
For one-to-one structural changes, Axon Framework 5.2.0 already provides xref:message-transformation:index.adoc[event transformation], the successor to the single-event upcaster. See the xref:migration:paths/upcasters.adoc[upcaster migration path].
The broader cases described below, such as splitting one event into several, merging events, and context-aware upcasting, are not yet covered.
endif::[]
...
ifdef::advanced-framework[]
+
NOTE: For one-to-one changes, xref:message-transformation:index.adoc[event transformation] is already available. The `SingleEventUpcaster` covers this case once the full upcaster API is reintroduced.
endif::[]
