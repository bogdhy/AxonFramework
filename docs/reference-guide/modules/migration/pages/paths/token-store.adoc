...
Axon Framework 4's `MultiSourceTrackingToken`, written by processors that read from more than one event source, has no equivalent in Axon Framework 5 and is not mapped.
These tokens cannot be read after upgrading.
Reset the affected processors so they rebuild their position, as described in <<option-2-reset-processors-to-latest,reset the processors to latest>>.
